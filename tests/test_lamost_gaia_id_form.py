...
def test_standardize_gaia_id_response_retains_only_requested_exact_ids() -> None:
    body = (
        ...
        b"999999999999999999|1002|59001.5|15.0|1.2|20|21|19|0|STAR|K0\n"
    )
    ...
    assert len(epochs) == 1
    assert int(epochs.iloc[0]["source_id"]) == 2676113965163724160
    assert bool(epochs.iloc[0]["success"])
    ...
    assert record["exact_identity_row_count"] == 1
