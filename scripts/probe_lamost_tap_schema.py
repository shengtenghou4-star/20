...
def main() -> None:
    ...
    tables = service.run_sync(table_query, maxrec=args.max_tables + 1).to_table().to_pandas()
    tables.columns = [str(column).lower() for column in tables.columns]
    if len(tables) >= args.max_tables:
        raise RuntimeError("TAP table discovery reached max_tables and may be truncated")
    ...
    if candidates.empty:
        raise RuntimeError("no LAMOST DR8/multiple-epoch TAP tables were discovered")
    ...
    literals = ",".join("'" + name.replace("'", "''") + "'" for name in candidate_names)
    column_query = (
    ...
    columns = service.run_sync(column_query, maxrec=20_000).to_table().to_pandas()
    ...
    for table_name, group in columns.groupby("table_name", sort=True):
        available = sorted(set(group["column_name"].astype(str).str.lower()))
        column_summary[str(table_name)] = available

    payload = {
        "status": "pass",
        "tap_url": args.tap_url,
        "table_query_sha256": hashlib.sha256(table_query.encode("utf-8")).hexdigest(),
        "column_query_sha256": hashlib.sha256(column_query.encode("utf-8")).hexdigest(),
        ...
        "candidate_tables": [
            {
                ...
                "recognized_contract_columns": sorted(
                    required_names & set(column_summary.get(str(row.table_name), []))
                ),
            }
            for row in candidates.itertuples(index=False)
        ],
        "claim_boundary": (
            "This metadata probe returns public table and column names only. It does not "
            "query source rows or establish catalogue overlap."
        ),
    }
